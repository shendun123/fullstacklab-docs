# GPT Image 2 / image2 图像生成

平台提供同步和异步两类 GPT 图像接口。先根据模型名和返回格式选择路径，不要混用不同路径的参数。

> 说明：有些兼容通道可能会直接返回 `data`、`url` 或 `b64_json`，这种属于同步返回格式；但如果返回的是 `task_id` / `id`，就必须按异步任务方式继续查询结果。

## 支持模型

| 模型 | 说明 |
| --- | --- |
| image2 | OpenAI 原生格式同步生图和图片编辑 |
| gpt-image-2 | 标准图像生成模型 |
| gpt-image-2-pro | 新版 Images 异步兼容路径中的高质量模型 |
| gpt-image-2-2K | `/v1/videos` 异步接口 2K 档，`K` 必须大写 |
| gpt-image-2-4K | `/v1/videos` 异步接口 4K 档，`K` 必须大写 |

`model` 以后台实际可用模型为准。

## 接口路径选择

| 调用方式 | 创建路径 | 查询路径 | 模型示例 |
| --- | --- | --- | --- |
| 同步生成 | `POST /v1/images/generations` | 不需要 | `image2` |
| 同步编辑 | `POST /v1/images/edits` | 不需要 | `image2` |
| 通用任务式 | `POST /v1/videos` | `GET /v1/videos/{task_id}` | `gpt-image-2`、`gpt-image-2-2K`、`gpt-image-2-4K` |
| 新版 Images 异步 | `POST /v1/images/generations/async` | `GET /v1/images/generations/async/{taskId}` | `gpt-image-2`、`gpt-image-2-pro` |

### image2 同步示例

```bash
curl "https://fullstacklab.xyz/v1/images/generations" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "image2",
    "prompt": "高级商业产品海报，柔和棚拍光线",
    "size": "1024x1792",
    "image": ["https://example.com/reference.jpg"]
  }'
```

本地文件编辑请使用 `multipart/form-data` 调用 `/v1/images/edits`。

### `/v1/videos` 异步示例

`aspect_ratio` 与 `size` 二选一；`images` 支持 URL 或完整 Data URL，最多 5 张。

```bash
curl "https://fullstacklab.xyz/v1/videos" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-image-2-2K",
    "prompt": "湖边日出，电影感，自然光，高细节",
    "aspect_ratio": "16:9"
  }'
```

该接口通过 `GET /v1/videos/{task_id}` 查询，完成后通常从 `url` 读取图片。生成链接有效期可能较短，请及时保存。

## 一、异步任务式流程

```text
1. POST 创建生图任务
2. 从返回结果中读取 task_id 或 id
3. GET 查询任务状态
4. status=completed 时读取 url / image_url / output / data
5. 下载图片
```

## 二、创建任务

```text
POST https://fullstacklab.xyz/v1/images/generations
```

如果后台使用新版异步路径，也可以使用：

```text
POST https://fullstacklab.xyz/v1/images/generations/async
```

对应查询路径：

```text
GET https://fullstacklab.xyz/v1/images/generations/async/{taskId}
```

## 请求头

```text
Authorization: Bearer YOUR_API_KEY
Content-Type: application/json
```

## 请求参数

| 参数 | 类型 | 必填 | 示例 | 说明 |
| --- | --- | --- | --- | --- |
| model | string | 是 | gpt-image-2 | 图像模型名称 |
| prompt | string | 是 | 一只可爱的橘猫 | 图片提示词 |
| n | integer | 否 | 1 | 生成张数 |
| size | string | 否 | 1024x1024 | 图片尺寸 |
| quality | string | 否 | high | 质量等级：`low`、`medium`、`high`、`auto` |
| response_format | string | 否 | url | 返回格式：`url` 或 `b64_json` |
| image | array | 否 | ["https://example.com/ref.png"] | 参考图 URL 或 base64 数组 |

常见尺寸包括：

| 尺寸 | 说明 |
| --- | --- |
| 1024x1024 | 方图 |
| 1536x1024 | 横图 |
| 1024x1536 | 竖图 |
| 2048x2048 | 2K 方图 |
| 1920x1080 | 16:9 横图 |
| 1080x1920 | 9:16 竖图 |
| 3840x2160 | 4K 横图 |
| 2160x3840 | 4K 竖图 |

## 三、创建任务示例

### Shell

```bash
curl -X POST "https://fullstacklab.xyz/v1/images/generations/async" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-image-2-pro",
    "prompt": "A cute orange cat playing with yarn, commercial photography style",
    "n": 1,
    "size": "1536x1024",
    "quality": "high",
    "response_format": "url"
  }'
```

### Python

```python
import requests

API_KEY = "YOUR_API_KEY"
BASE_URL = "https://fullstacklab.xyz/v1"

headers = {
    "Authorization": f"Bearer {API_KEY}",
    "Content-Type": "application/json"
}

payload = {
    "model": "gpt-image-2-pro",
    "prompt": "A cute orange cat playing with yarn, studio ghibli style",
    "n": 1,
    "size": "1536x1024",
    "quality": "high",
    "response_format": "url"
}

res = requests.post(
    f"{BASE_URL}/images/generations/async",
    headers=headers,
    json=payload,
    timeout=120
)

print("状态码:", res.status_code)
print("返回内容:", res.text)

res.raise_for_status()
result = res.json()

task_id = result.get("task_id") or result.get("id")
print("任务 ID:", task_id)

if not task_id:
    print("当前通道可能是同步返回，请检查 data / url / b64_json 字段")
```

### Java

```java
import java.net.URI;
import java.net.http.HttpClient;
import java.net.http.HttpRequest;
import java.net.http.HttpResponse;

public class GptImageExample {
    public static void main(String[] args) throws Exception {
        String apiKey = "YOUR_API_KEY";
        String body = """
            {
              "model": "gpt-image-2-pro",
              "prompt": "A cute orange cat playing with yarn, commercial photography style",
              "n": 1,
              "size": "1536x1024",
              "quality": "high",
              "response_format": "url"
            }
            """;

        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://fullstacklab.xyz/v1/images/generations/async"))
            .header("Authorization", "Bearer " + apiKey)
            .header("Content-Type", "application/json")
            .POST(HttpRequest.BodyPublishers.ofString(body))
            .build();

        HttpResponse<String> response = HttpClient.newHttpClient()
            .send(request, HttpResponse.BodyHandlers.ofString());

        System.out.println(response.body());
    }
}
```

## 四、轮询任务结果

如果创建任务返回了 `task_id` 或 `id`，继续轮询：

```python
import time
import requests

API_KEY = "YOUR_API_KEY"
BASE_URL = "https://fullstacklab.xyz/v1"

def wait_image_result(task_id, interval=3, timeout=420):
    url = f"{BASE_URL}/images/generations/async/{task_id}"
    headers = {
        "Authorization": f"Bearer {API_KEY}"
    }

    start = time.time()

    while True:
        res = requests.get(url, headers=headers, timeout=60)
        print("查询状态码:", res.status_code)
        print("查询返回:", res.text)

        res.raise_for_status()
        data = res.json()

        status = str(data.get("status", "")).lower()
        progress = data.get("progress", 0)

        print("当前状态:", status, "进度:", progress)

        if status == "completed":
            return (
                data.get("url")
                or data.get("image_url")
                or data.get("output")
                or data.get("data")
            )

        if status == "failed":
            raise Exception(data.get("error") or data.get("fail_reason") or data)

        if time.time() - start > timeout:
            raise TimeoutError(f"任务超时，超过 {timeout} 秒仍未完成")

        time.sleep(interval)
```

## 五、保存图片

轮询完成后，可能返回图片 URL，也可能返回包含图片信息的对象。可以按下面方式兼容：

```python
import base64
import time
from pathlib import Path
import requests

def save_image(result, save_dir="./generated_images"):
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)

    # 情况 1：直接是图片 URL
    if isinstance(result, str) and result.startswith("http"):
        img = requests.get(result, timeout=120)
        img.raise_for_status()
        path = save_dir / f"image_{int(time.time())}.png"
        path.write_bytes(img.content)
        return path

    # 情况 2：返回 data 数组
    if isinstance(result, dict):
        items = result.get("data", [])
        for index, item in enumerate(items, start=1):
            if "url" in item:
                img = requests.get(item["url"], timeout=120)
                img.raise_for_status()
                path = save_dir / f"image_{index}_{int(time.time())}.png"
                path.write_bytes(img.content)

            elif "b64_json" in item:
                data = base64.b64decode(item["b64_json"])
                path = save_dir / f"image_{index}_{int(time.time())}.png"
                path.write_bytes(data)
```

## 六、同步返回兼容说明

如果你的请求没有返回 `task_id` 或 `id`，而是直接返回：

```text
data
url
b64_json
```

说明当前通道是同步返回，可以直接保存图片，不需要轮询。

## 七、排查建议

| 现象 | 可能原因 | 处理方式 |
| --- | --- | --- |
| 返回 `task_id` 但没有图片 | 异步任务还没完成 | 继续轮询任务结果 |
| 一直 `queued` / `processing` | 上游排队或处理慢 | 增加轮询总超时时间 |
| `failed` | 提示词、模型、权限或上游失败 | 打印 `error` / `fail_reason` |
| 请求超时 | 生图耗时较长 | 把 timeout 调到 120 秒或 300 秒 |
| 返回没有 `url` | 字段格式不同 | 同时兼容 `url`、`image_url`、`output`、`data`、`b64_json` |

## 八、安全提醒

示例代码中的：

```text
YOUR_API_KEY
```

需要替换成你自己在全栈实验室后台创建的真实 Key。不要把真实 Key 写进 GitHub、公开文档、截图或群聊。
